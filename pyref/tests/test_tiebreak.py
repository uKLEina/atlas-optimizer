...
2フェーズ辞書式のため、重みはポイント最適性に一切影響しない
(フェーズ1が従来と同一の問題を解き、フェーズ2は同点制約の下で重みだけを最小化)。
"""
...
def diamond() -> AtlasGraph:
    ...
    同点チェーンの選択が縮約段階((w, eps) 辞書式比較)で正しく決まることも検証できる。
    """
    adj = {
        "r": frozenset({"a1", "b1"}),
        "a1": frozenset({"r", "a2"}),
...
def test_prefers_low_weight_route():
    g = diamond()
    res = solve(g, ["t"], node_weights={"a1": 7, "a2": 7, "b1": 1, "b2": 1})
    assert res.status == "optimal"
    assert res.points == 3
    assert res.nodes == frozenset({"r", "b1", "b2", "t"})
    # 重みを逆にすれば逆の経路が選ばれる
    res2 = solve(g, ["t"], node_weights={"a1": 1, "a2": 1, "b1": 7, "b2": 7})
...
def test_large_weights_cannot_break_optimality():
    # 重みがどれだけ大きくても遠回り(4ポイント経路)は選ばれない。
    # c1-c2-c3 の迂回路を追加し、最短側に巨大な重みを付ける
    adj = {
        "r": frozenset({"a1", "c1"}),
        "a1": frozenset({"r", "t"}),
        "c1": frozenset({"r", "c2"}),
    ...
    res = solve(g, ["t"], node_weights={"a1": 10_000})
    assert res.status == "optimal"
    assert res.points == 2  # r(コスト0) - a1 - t
    assert res.nodes == frozenset({"r", "a1", "t"})
...
def test_points_unchanged_on_real_graph(g):
    ...
    weights = {n: rnd.choice([0, 1, 3, 7]) for n in g.adj if n != g.root}
